...
async def select_all(message: types.Message):
    ...
    await MessageState.message9.set()
...
async def select_pvz(message: types.Message, state: FSMContext):
    ...
    await message.answer("Напиши день за который хочешь заказы.\n"
                         "Формат: 2022-09-29 (Год-месяц-число)\n"
                         "\nМожешь вводить данные:")
    await MessageState.message10.set()
...
async def get_info_amount(message: types.Message, state: FSMContext):
    ...
    pvz_dec = data_a["pvz_select"]
    date_v = data_a["date_select"]
    date_dec = datetime.strptime(date_v, "%Y-%m-%d").date()

    order = await db.select_orders_acc(f_pvz=pvz_dec,
                                       insert_day=date_dec)
...
def register_handlers_get(dp: Dispatcher):
    dp.register_message_handler(select_all, commands="get", state="*")
    dp.register_message_handler(select_pvz, state=MessageState.message9)
    dp.register_message_handler(get_info_amount, state=MessageState.message10)